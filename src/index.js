...
const getCoinsPrice = async (tickers, buttons) => {
  const res = await axios.get(
    `https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=${tickers.join(
      ","
    )}`,
    {
      headers: {
        // Change value to your Coinmaret cap api key.
  ...
  Object.keys(buttons).forEach((buttonTicker) => {
    ...
    if (
      data[buttonTickerUppercase] &&
      data[buttonTickerUppercase][0].quote?.USD?.price
    ) {
      buttons[buttonTicker].label = `${buttonTickerUppercase} ${data[
        buttonTickerUppercase
      ][0].quote?.USD?.price.toFixed(2)}$`;
    } else {
